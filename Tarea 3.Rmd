...
**En base a la descripciòn que realiza con respecto a las preguntas se concluye que es un Anàlisis descriptivo, porque el objetivo de este es caracterizar y resumir un conjunto de datos mediante estadisticas descriptivas**.
...
**En base a la descripciòn que realiza con respecto a las preguntas se concluye que es un Análisis predictivo debido a que el interès central radica en modelar patrones históricos para anticipar la probabilidad de fallos futuros asociados a una red específica y contrastarlos con otras redes.**.
...
**En base a la descripciòn que realiza con respecto a las preguntas se concluye que es un análisis exploratorio debido a que se busca identificar estructuras latentes o patrones de segmentación en los datos mediante técnicas de agrupamiento, sin una variable dependiente previamente definida.**.
...
**Consideramos que para abordar el escenario a travès de Data Science, debemos contemplar un anàlisis basado en los logs històricos de conexiones TCP salientes de todos los equipos corporativos, con el fin de poder identificar cuales son estos equipos y tener el listado de usuarios que son posibles sospechosos para poder reducir nuestro universo de casos. Cabe mencionar que estos registros deben ser obtenidos tanto de los sistemas operativos, SIEM, herramientas de monitorizaciòn, firewall, entre otros que permitan analizar las direcciones IP considerand los puertos utilizados.**<br> <br> Posterior al paso mencionado, se debe realizar una limpieza de datos "datos elegantes", en el cual se tiene que depurar los registros incompletos o duplicados con el fin de enriquecer la informaciòn a travès de la identificaciòn de puertos permitidos, rangos de IP legìtimos de clientes, y horarios de jornadas laborales. Esta actividad es crìtica para poder determinar la identificaciòn del comportamiento atìpicos.<br> <br> Con respecto a las preguntas, podemos considerar preguntas como: què tipo de equipos establecieron conexiones a servicios no autorizados, el tiempo que tuvo esa conexiòn, asì como los patrones temporales anòmalos y si las direcciones IP externas corresponden a ubicaciones habituales del negocio.<br> <br> Para ello, pueden aplicarse técnicas de análisis exploratorio y detección de anomalías, con el fin de reducir el conjunto de posibles equipos implicados. Y asì los resultados se presentarían mediante visualizaciones como distribuciones de uso de puertos o series temporales de conexiones, destacando los riesgos observados y reforzando la importancia del cumplimiento de las políticas de seguridad, sin adoptar un enfoque culposo.
